...
#[expect(
    ...
    reason = "Not a state machine, but could be replaced with a packet bit set in the future."
)]
pub struct DisplayedSkinParts {
    ...
    /// [`DisplayedSkinParts`] field boolean, where the value is:
    /// - `true` if the player's right sleeve is visible.
    /// - `false` if the player's right sleeve is not visible.
    ///
    /// **Protocol mask: `0x_08`**, `0b_0000_1000` (fourth-to-least significant bit).
    pub right_sleeve: bool,

    /// [`DisplayedSkinParts`] field boolean, where the value is:
    /// - `true` if the player's left pants/trousers leg is visible.
    /// - `false` if the player's left pants/trousers leg is not visible.
    ///
    ...
    /// [`DisplayedSkinParts`] field boolean, where the value is:
    /// - `true` if the player's hat is visible.
    /// - `false` if the player's hat is not visible.
    ///
    /// **Protocol mask: `0x_40`**, `0b_0100_0000` (seventh-to-least significant bit).
    pub hat: bool,
}
