...
pub fn main() {
    ...
    app.insert_resource(Motd("Check your username by joining this server".to_owned()));
    app.insert_resource(OnlinePlayers(987654321));
    app.insert_resource(MaxPlayers(1234567890));
...
fn login_listener(mut pr: MessageReader<LoginStart>, mut pw: MessageWriter<Disconnect>) {
    for packet in pr.read() {
        pw.write(Disconnect {
            ...
            reason: JsonText {
                data: format!("{{text: \"Your username is {} \"}}", packet.name)
            },
        });
    }
}
