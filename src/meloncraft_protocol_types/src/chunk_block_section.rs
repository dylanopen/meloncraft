...
impl ProtocolType for ChunkBlockSection {
    ...
    fn net_deserialize(_data: &mut Vec<u8>) -> Result<Self, ()> {
        panic!(
            "Tried to deserialize a ChunkBlockSection, but this struct can't currently be deserialized. If you need this struct to be deserialized, please open an issue or, better yet, a pull request that implements deserialization for this struct."
        );
    }
}
