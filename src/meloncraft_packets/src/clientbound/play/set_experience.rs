...
impl ClientboundPacket for ClientboundSetExperience {
    ...
    fn data(&self, data: &mut Vec<u8>) {
        let bar = self.bar.unwrap_or_else(|| return experience::total_to_level(self.total.0) % 1.0);
        let level = self.level.unwrap_or_else(|| return experience::total_to_level(self.total.0) as i32);
