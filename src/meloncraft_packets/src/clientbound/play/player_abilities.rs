...
use meloncraft_player::{FlySpeed, FovModifier};
use crate::network_messages::ClientboundNetworkPacket;
use meloncraft_protocol_types::ProtocolType as _;
use crate::clientbound_packet::ClientboundPacket;

/// Send the client their 'abilities', see the fields for what this includes.
#[expect(clippy::struct_excessive_bools, reason = "Bools are fine for now, because some ability bitflag component structs hold no value")]
...
impl ClientboundPacket for ClientboundPlayerAbilities {
    ...
    fn serialize(&self) -> Option<ClientboundNetworkPacket> {
        ...
        data.extend((
                u8::from(self.invulnerable)
                | (u8::from(self.is_flying) << 1)
                | (u8::from(self.allow_flying) << 2)
                | (u8::from(self.instant_break) << 3)
        ).net_serialize());
